...
let string_of_type (t: typeScheme) =
  let rec aux (t: typeScheme) =
    ...
    | TPoly(vars, t_inner) ->
        ...
        Printf.sprintf "forall %s. %s" vars_str (aux t_inner)
  in
  aux t
;;
...
let pp_string_of_type (t: typeScheme) =
  let rec aux (t: typeScheme) (chr: int) (map) =
    match t with
    | TNum -> "int", chr, map
    | TBool -> "bool", chr, map
    | TStr -> "string", chr, map
    | T(x) ->
        if List.mem_assoc x map then
          let gen_chr = List.assoc x map in
          Printf.sprintf "'%s" gen_chr, chr, map
        else
          let gen_chr = Char.escaped (Char.chr chr) in
          let new_map = (x, gen_chr)::map in
          let new_chr = chr + 1 in
          Printf.sprintf "'%s" gen_chr, new_chr, new_map
    | TFun(t1, t2) ->
        let (st1, c1, m1) = aux t1 chr map in
